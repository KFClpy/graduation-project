...
class AutoFJGreedyAlgorithm(object):
    ...
    def scale_candidate_thresholds(self, unscaled_thresholds):
        ...
        for jf in self.join_functions:
            ...
            if self.threNew[jf] != 0 and (max_d-min_d) / (self.threNew[jf]-min_d) < 10 :
                cand_threshNew = [thresh for thresh in cand_thresh if thresh < self.threNew[jf]]
            else :
                cand_threshNew=cand_thresh.tolist()
            candidate_thresholds[jf] = set(cand_threshNew)
