...
from service.userListService import get_user_list, delete_user, edit_user, search_user
...
search_parser.add_argument("user_name", help="用户名不能为空", required=True)
...
class SearchUser(Resource, BaseView):
    @jwt_required()
    def post(self):
        user_name = get_jwt_identity()
        name = search_parser.parse_args()['user_name']
        try:
            data = {
                "userList": search_user(name,user_name)
            }
            return self.formattingData(code=Codes.SUCCESS.code, msg=Codes.SUCCESS.desc, data=data)
        except Exception as e:
            base_log.info(e)
